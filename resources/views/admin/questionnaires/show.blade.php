						...
						<details open>
							...
							@foreach( $evaluatingQuestion->where('group_id', $evaluatingGroup->id) as $question )
								<div class="pitanje">
									...
									<span class="ocj">							
										@foreach($evaluatingRatings as $evaluatingRating)
											<input type="radio" name="rating[{{ $question->id }}]" value="{{ $evaluatingRating->rating }}" id="myRadio1" /><b>{{ $evaluatingRating->rating }}</b>
										@endforeach 
									</span>
					...
					@foreach($evaluatingGroups as $evaluatingGroup)
						<details open>
							<summary id="{{ $evaluatingGroup->naziv }}">{{ $evaluatingGroup->naziv }}</summary>
								...
								<span class="ocj">									
									@foreach($evaluatingRatings as $evaluatingRating)
										<input type="radio" name="rating[{{ $evaluatingGroup->id }}]" value="{{ $evaluatingRating->rating }}" id="myRadio2"  /><b>{{ $evaluatingRating->rating }}</b>
									@endforeach 
								</span>
							@foreach( $evaluatingQuestion->where('group_id', $evaluatingGroup->id) as $question )
