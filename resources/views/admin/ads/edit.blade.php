			...
			<div class="panel-body">
				 <form accept-charset="UTF-8" role="form" method="post" enctype="multipart/form-data" action="{{ route('admin.ads.store') }}">
					<div class="form-group {{ ($errors->has('subject'))  ? 'has-error' : '' }}">
                        <label>Naslov</label>
						<input name="subject" type="text" class="form-control" value="{{ $ad->subject }}" required >
						{!! ($errors->has('subject') ? $errors->first('subject', '<p class="text-danger">:message</p>') : '') !!}
                    </div>
					<div class="form-group {{ ($errors->has('description')) ? 'has-error' : '' }}">
						<label>Obavijest:</label>
						<textarea id="summernote" name="description" required>{{ $ad->description }}</textarea>
						{!! ($errors->has('description') ? $errors->first('description', '<p class="text-danger">:message</p>') : '') !!}
					</div>
					<input name="category_id" value="{{ $ad->category_id }}" type="hidden">
					<div class="form-group {{ ($errors->has('price'))  ? 'has-error' : '' }}">
                        <label>Cijena</label>
						<input name="price" type="text" class="form-control" value="{{ $ad->price }}" required hidden >
