...
<div class="page-header">
  <h2> {{$evaluatingEmployee->questionnaire['naziv'] }} za mjesec {{$evaluatingEmployee->mjesec_godina }}</h2>
</div> 
<div class="row">
	<div class="col-12">
					...
					<div class="box form-group ">
						...
						?>
							@if(!DB::table('employee_terminations')->where('employee_id',$registration->employee_id)->first())
								<span class="ev_empl"><input type="checkbox" name="ev_employee_id[]" value="{{ $registration->employee_id }}" {!! $registration->employee_id == $value ? 'checked' : '' !!} disabled />{{ $registration->employee['first_name'] . ' ' . $registration->employee['last_name'] }}</span>
							@endif
						@endforeach
					</div>	
