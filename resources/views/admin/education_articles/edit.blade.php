	...
	<div class="col-md-8 col-md-offset-2 col-lg-6 col-lg-offset-3">
		<div class="panel panel-default">
			<div class="panel-body">
				 <form accept-charset="UTF-8" role="form" method="post" action="{{ route('admin.education_articles.update', $educationArticle->id) }}">
					<div class="form-group {{ ($errors->has('theme_id'))  ? 'has-error' : '' }}">
						...
						<select  class="form-control"  name="theme_id" required >
							...
							@foreach ($educationThemes as $educationTheme)
								<option value="{{ $educationTheme->id }}"  {!! $educationTheme->id == $educationArticle->theme_id ? 'selected' : '' !!} >{{ $educationTheme->name }}</option>
							@endforeach
						</select>
					...
					<div class="aktivna form-group {{ ($errors->has('status'))  ? 'has-error' : '' }}">
                        <label>Status</label>
						<input type="radio" class="" name="status" value="neaktivan" {!! $educationArticle->status == 'neaktivan' ? 'checked' : '' !!} />NEAKTIVAN
						<input type="radio" class="" name="status" value="aktivan" {!! $educationArticle->status == 'aktivan' ? 'checked' : '' !!} />AKTIVAN
