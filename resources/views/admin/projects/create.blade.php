	...
	<div class="panel-body">
		<form accept-charset="UTF-8" role="form" method="post" action="{{ route('admin.projects.store') }}">
			<div class="form-group {{ ($errors->has('erp_id')) ? 'has-error' : '' }}">
				...
				<input class="form-control" placeholder="Broj projekta" name="erp_id" type="text" value="{{ old('erp_id') }}" required />
			...
			<div class="form-group {{ ($errors->has('customer_oib')) ? 'has-error' : '' }}">
				...
				<input class="form-control" placeholder="OIB naručitelja" name="customer_oib" type="text" value="{{ old('customer_oib') }}" required/>
				{!! ($errors->has('customer_oib') ? $errors->first('customer_oib', '<p class="text-danger">:message</p>') : '') !!}
			</div>
			
			<div class="form-group {{ ($errors->has('naziv')) ? 'has-error' : '' }}">
				<text>Naziv projekta</text>
			   <textarea class="form-control"  placeholder="Naziv projekta"  name="naziv" id="projekt-name" required>{{ old('naziv') }}</textarea>
