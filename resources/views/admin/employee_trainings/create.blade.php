	...
	<div class="col-md-8 col-md-offset-2 col-lg-6 col-lg-offset-3">
		<div class="panel panel-default">
			<div class="panel-body">
				 <form accept-charset="UTF-8" role="form" method="post" action="{{ route('admin.employee_trainings.store') }}">
						...
						<select class="form-control" name="employee_id" id="sel1" value="{{ old('employee_id') }}">
							...
							@foreach ($registrations as $registration)
								@if(! $terminations->where('employee_id', $registration->employee_id)->first() )
									<option name="employee_id" value="{{ $registration->employee_id }}">{{ $registration->last_name  . ' ' . $registration->first_name }}</option>
								@endif
							@endforeach	
						</select>
					...
					<div class="form-group {{ ($errors->has('training_id'))  ? 'has-error' : '' }}">
						...
						<select class="form-control" name="training_id" id="sel1" value="{{ old('training_id') }}">
							...
							@foreach ($trainings as $training)
								<option name="training_id" value="{{ $training->id }}">{{ $training->name }}</option>
							@endforeach	
						</select>
					...
					<div class="form-group {{ ($errors->has('expiry_date')) ? 'has-error' : '' }}">
						<label>Datum uvjerenja:</label>
						<input name="expiry_date" type="date" class="form-control" value ="{{ Carbon\Carbon::now()->format('Y-m-d') }}">
						{!! ($errors->has('expiry_date') ? $errors->first('expiry_date', '<p class="text-danger">:message</p>') : '') !!}
					</div>
					<div class="form-group {{ ($errors->has('description')) ? 'has-error' : '' }}">
						<label>Napomena:</label>
						<textarea name="description" type="text" rows="3" maxlength="255" class="form-control"></textarea>
