...
class GO extends Command
{
    ...
    public function handle()
    {
		...
		$izostanci_priv = TemporaryEmployeeRequest::join('temporary_employees','temporary_employee_requests.employee_id', '=', 'temporary_employees.id')->select('temporary_employee_requests.*', 'temporary_employees.first_name','temporary_employees.last_name')->orderBy('temporary_employee_requests.zahtjev','ASC')->orderBy('temporary_employees.last_name','ASC')->get();
		
		if(count($izostanci) > 0) {
			foreach($izostanci as $izostanak){
				$ukupno_GO = 0;
				$ukupnoDani = 0;
				$prijenos_zahtjeva = 0;
				if($izostanak->odobreno == 'DA'){
					$begin1 = new DateTime($izostanak->start_date);
					$end1 = new DateTime($izostanak->end_date);
					$end1->setTime(0,0,1);
					...
					if($izostanak->zahtjev == 'Izlazak'){
						if($begin_dan == $dan && $begin_mjesec == $mjesec  && $begin_godina == $ova_godina){
							array_push($dan_izostanci,array('ime' => $izostanak->first_name . ' ' . $izostanak->last_name, 'zahtjev' =>  $izostanak->zahtjev, 'period' => date('d.m.Y', strtotime( $izostanak->start_date )) . ' - ' .  date('d.m.Y', strtotime($izostanak->end_date)), 'vrijeme' => $izostanak->start_time . ' - ' .  $izostanak->end_time,  'napomena' =>  $izostanak->napomena, 'GO' => '', 'ukupnoGO' => ''));
						}
					} else {
						...
						if($begin1 == $end1 && $begin_dan == $dan && $begin_mjesec == $mjesec){
							array_push($dan_izostanci,array('ime' => $izostanak->first_name . ' ' . $izostanak->last_name, 'zahtjev' =>  $izostanak->zahtjev, 'period' => date('d.m.Y', strtotime( $izostanak->start_date)), 'vrijeme' => $izostanak->start_time . ' - ' .  $izostanak->end_time, 'dani_GO' => $dani_GO, 'napomena' =>  $izostanak->napomena ));
						} else {
							if($izostanak->first_name . ' ' . $izostanak->last_name != 'Nikolina Gunčić') {          /* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
								foreach ($period1 as $dan1) {  //ako je dan  GO !!!
									...
									if($period_day == $dan && $period_month == $mjesec && $period_year == $ova_godina ){ //|| $begin1 == $end1 
										array_push($dan_izostanci,array('ime' => $izostanak->first_name . ' ' . $izostanak->last_name, 'zahtjev' =>  $izostanak->zahtjev, 'period' => date('d.m.Y', strtotime( $izostanak->start_date)) . ' - ' .  date('d.m.Y', strtotime($izostanak->end_date)), 'vrijeme' => $izostanak->start_time . ' - ' .  $izostanak->end_time, 'napomena' =>  $izostanak->napomena, 'dani_GO' => $dani_GO));
									}
								}
							}						
						}
				...
				if($izostanak_priv->odobreno == 'DA'){
					...
					foreach ($period2 as $dan2) {  //ako je dan  GO !!!
						...
						if($begin2 == $end2 && $begin_dan == $dan && $begin_mjesec == $mjesec){
							array_push($dan_izostanci,array('ime' => $izostanak_priv->first_name . ' ' . $izostanak_priv->last_name, 'zahtjev' =>  $izostanak_priv->zahtjev, 'period' => date('d.m.Y', strtotime( $izostanak_priv->start_date)), 'vrijeme' => $izostanak_priv->start_time . ' - ' .  $izostanak_priv->end_time, 'dani_GO' => $dani_GO, 'napomena' =>  $izostanak_priv->napomena ));
						} else if($period_day == $dan && $period_month == $mjesec && $period_year == $ova_godina || $begin2 == $end2 ){
							array_push($dan_izostanci,array('ime' => $izostanak_priv->first_name . ' ' . $izostanak_priv->last_name, 'zahtjev' =>  $izostanak_priv->zahtjev, 'period' => date('d.m.Y', strtotime( $izostanak_priv->start_date)) . ' - ' .  date('d.m.Y', strtotime($izostanak_priv->end_date)), 'vrijeme' => $izostanak_priv->start_time . ' - ' .  $izostanak_priv->end_time, 'napomena' =>  $izostanak_priv->napomena, 'dani_GO' => '-'));
						}
					}
				}
			}
