...
class TaskNotificationEmails extends Command
{
    /**
                ...
                if( date_format($task_date, 'Y-m-d') == date_format($today, 'Y-m-d') ) {
                    ...
                    if(count( $employee_ids ) > 1) {
                        ...
                        if($prev_task) {
                            ...
                            $response = GodisnjiController::go_na_dan_odobreni($empl_id, date_format($task_date, 'Y-m-d'));  // djelatnik ima zahtjev na dan (a nije Izlazak)
                            if($response == false) {
                                if ( $key == (count( $employee_ids ) -1) ) {
                                    $key = 0;
                                } else {
                    ...
                    if( $email != null && $email != '') {
                        Mail::queue('email.task', ['employeeTask' => $employeeTask ], function ($mail) use ($task, $email) {
                            $mail->to($email)
                                ->from('info@duplico.hr', 'Duplico')
                                ->subject('Podsjetnik na zadatak');
                        });
                        /*   Mail::queue('email.task', ['employeeTask' => $employeeTask ], function ($mail) use ($task) {
